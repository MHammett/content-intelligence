...
- **Quality trends** — Flesch-Kincaid grade, SEO issue count, and broken link count, both globally (recent vs. baseline average) and per article across its own revision history (first measurement vs. latest, as `improved` / `worsened` / `unchanged`, or `unknown` when no run measured it). Each metric counts only the runs that measured it.
...
A run that never checked its links — `--offline`, or `link_validation: false` — has no broken-link count, rather than a count of zero. Its report writes `pre_analysis.links` as `null`, with `links_skipped_reason` saying which (`offline` or `disabled`), and the console summary prints `Links: not checked`. The link trends leave it out entirely, recent window included, so it cannot turn them. A report from before 2026-09-18 wrote an empty list for such a run, which is also what a checked draft with no links gets. Nothing in such a report says which it was, so it counts as zero broken.
