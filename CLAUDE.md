...
- **`balanced` is not a good value** — 6.4% repro is statistically indistinguishable from `wide`'s 6.8% at 3.4x the cost — so it is no longer the shipped default: the same change (PR #182, 2026-09-08) switched `packages/ci-article-review/src/ci_article_review/configs/user.example.yaml` from `cost_preset: balanced` to `cost_preset: wide`. `balanced` itself was kept unchanged, not retired, and configs that name it still run it; retiring or retuning the tier is a separate, larger decision than this measurement made on its own.
