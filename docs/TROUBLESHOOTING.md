...
**Link shows `BROKEN (TLS certificate could not be verified: …)`**  
The server answered, but its certificate did not verify, so the page was never requested — the text after the colon is the verifier's reason (`unable to get local issuer certificate`, `certificate has expired`, a hostname mismatch). This is not a sign the page is missing, and the summary lists it with the other unread links, not with confirmed 404s. Open it in a browser. If the browser warns too, the certificate really is bad and every reader will see that warning — cite an archived copy or another source. If the browser loads it cleanly, the machine running the pipeline trusts less than your browser does; see [CITATIONS.md](CITATIONS.md#certificate-verification). An archive.org snapshot is tried either way, and a recovered link reads `OK (via archive: origin's TLS certificate could not be verified)`.

**Link shows `OK (via archive: …)`**  
The live URL could not be read — the parenthetical says why (`403 blocked`, `origin timed out`, `origin unreachable`, `origin's TLS certificate could not be verified`, `401 auth required`, `429 rate limited`) — but archive.org had a snapshot the pipeline could read. The link is not confirmed working right now, only confirmed to have existed. If the snapshot is also flagged `[STALE]`, the content you're citing may have moved on. Worth a manual look before publishing, especially for a primary source.
